...
if "%choice%" == "5" set BOARD_ID=NUCLEO-U5A5ZJ-Q

:next
:: Check if the board ID is recognized (NUCLEO-U575ZI-Q, NUCLEO-H7S3L8, NUCLEO-H563ZI or NUCLEO-N657X0-Q)
if not "%BOARD_ID%" == "NUCLEO-H7S3L8" if not "%BOARD_ID%" == "NUCLEO-U575ZI-Q" if not "%BOARD_ID%" == "NUCLEO-H563ZI" if not "%BOARD_ID%" == "NUCLEO-N657X0-Q" if not "%BOARD_ID%" == "NUCLEO-U5A5ZJ-Q" (
...
if "%BOARD_ID%" == "NUCLEO-N657X0-Q" (
    ...
    STM32_Programmer_CLI.exe -c port=swd mode=ur -w %current_dir%NUCLEO-U575ZI-Q_Binaries\Bootloader.bin 0x08000000 --go
    if %ERRORLEVEL% neq 0 goto :error
)
...
if "%BOARD_ID%" == "NUCLEO-N657X0-Q" (
    ...
    STM32_Programmer_CLI.exe -c port=swd mode=ur -w %current_dir%NUCLEO-U575ZI-Q_Binaries\ST67W6X_CLI.bin 0x08000000 --go
    if %ERRORLEVEL% neq 0 goto :error
)
